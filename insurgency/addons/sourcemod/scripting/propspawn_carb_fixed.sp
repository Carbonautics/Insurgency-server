...
public ScaleBuildPoints() {
	...
	for (new engineerCheck = 1; engineerCheck <= MaxClients; engineerCheck++)
	{
		if (engineerCheck > 0)
		{
			if (IsClientConnected(engineerCheck) && IsClientInGame(engineerCheck) && !IsFakeClient(engineerCheck) && (StrContains(g_client_last_classstring[engineerCheck], "recon") > -1))
			{
				tEngCount++;
...
public Action:Timer_Monitor_Props(Handle:Timer)
{
	...
	{
		//ENgineer specific
		if (IsClientConnected(client) && IsClientInGame(client) && !IsFakeClient(client) && (StrContains(g_client_last_classstring[client], "recon") > -1))
		{
					...
					for(new i=0; i<=iPropNo[engineerCheck]; i++)
					{
						...
						{	
							...
							if (tPropTarget != -1)
							{
								...
								if (iPropRef == mPropRef && fDistance <= 400 && !(StrContains(g_client_last_classstring[engineerCheck], "recon") > -1))
								{
								...
								if (iPropRef == mPropRef && fDistance <= 120 && (StrContains(g_client_last_classstring[client], "recon") > -1))
								{
...
public Event_PlayerSpawn(Handle: event , const String: name[] , bool: dontBroadcast)
{
	...
	//iCredits[client] = iDefCredits;
	if ((StrContains(g_client_last_classstring[client], "recon") > -1))
	{
...
public Event_PlayerDeath(Handle: event , const String: name[] , bool: dontBroadcast)
{
	...
	if ((StrContains(g_client_last_classstring[victim], "recon") > -1))
	{
...
public Action:Timer_Construct(Handle timer, Handle pack)
{
	...
	if (client > 0 && g_ConstructRemainingTime[client] <= 0 && IsPlayerAlive(client) && !IsClientTimingOut(client) && ((StrContains(sWeapon, "weapon_kabar") > -1) || (StrContains(sWeapon, "weapon_doi2ins_shovel") > -1)))
	{
		g_ConstructRemainingTime[client] = g_ConstructDeployTime;
...
OnButtonPress(client, button, buttons)
{
   ...
   if(button == IN_SPRINT && buttons & IN_DUCK && buttons & IN_CANCEL && (StrContains(g_client_last_classstring[client], "recon") > -1) && g_engInMenu[client] == false && ((StrContains(sWeapon, "weapon_kabar") > -1) || (StrContains(sWeapon, "weapon_doi2ins_shovel") > -1)))// && !(buttons & IN_FORWARD) && !(buttons & IN_ATTACK2) && !(buttons & IN_ATTACK))// & !IN_ATTACK2) 
   {
