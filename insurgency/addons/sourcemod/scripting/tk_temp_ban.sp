...
#define PLUGIN_NAME "[=F|A=] TeamKilling"
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(IsValidClient(victim) && IsValidClient(attacker))
	{
		if((!IsFakeClient(victim)) && (!IsFakeClient(attacker)) && (GetClientTeam(attacker) == GetClientTeam(victim)) && (victim != attacker))
		{
			...
			{
				PrintToChat(victim, "\x01\x0759b0f9[=F|A=] \x01\x07ee1fd0[TeamKill]\x01 Type /forgive in your chat to forgive your TKer. Otherwise, they get banned when they reach %i TKs.", g_nKickLimit);
			}
...
public Action:Cmd_TK_Info(client, args)
{
	...
	PrintToChat(client, "\x01\x0759b0f9[=F|A=] \x01\x07ee1fd0[TeamKill]\x01 Your total TK: %i of %i", g_nPlayerTKCounter[client], g_nKickLimit);
	PrintToChat(client, "\x01\x0759b0f9[=F|A=] \x01\x07ee1fd0[TeamKill]\x01 If you reach %i TKs, you will get ban for %s.", g_nKickLimit, strTemp);
...
{
	PrintToChat(client, "\x01\x0759b0f9[=F|A=] \x01\x07ee1fd0[TeamKill]\x01 Total TK %i, Total Forgive %i, Total Ban %i", g_nTotalTK, g_nTotalForgive, g_nTotalAutoBan);
	return Plugin_Handled;
}
