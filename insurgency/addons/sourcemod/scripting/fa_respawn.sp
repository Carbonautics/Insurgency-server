...
public OnPluginStart()
{
	...
	// Donor tag
	sm_respawn_enable_donor_tag = CreateConVar("sm_respawn_enable_donor_tag", "1", "If player has an access to reserved slot, add [VIP] tag.");
...
public Action:Timer_EnemyReinforce(Handle:Timer)
{
	...
	if (g_iRemaining_lives_team_ins <= (g_iRespawn_lives_team_ins / iReinforce_multiplier) + iReinforce_multiplier_base)
	{
		...
		if (g_iReinforceTime % 120 == 0 && g_iReinforceTime > 5)
		{
		...
		if (g_iReinforceTime <= 45 && (validAntenna != -1 || g_jammerRequired == 0))
		{
					...
					{ 
						...
						if (GetVectorDistance(fPlayerOrigin, fOrigin) <= Healthkit_Radius)
						{
							...
							else
							{
								...
								else if (iHealth < g_nonMedicHealSelf_max && !(StrContains(sWeapon, "weapon_knife") > -1) || !(StrContains(sWeapon, "weapon_kabar") > -1))
								{
