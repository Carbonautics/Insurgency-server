...
public OnPluginStart()
{
	...
	sm_respawn_mode_team_ins = CreateConVar("sm_respawn_mode_team_ins", "1", "Insurgents: 0 = Individual spawning | 1 = Wave based spawning");

	//Wave interval for insurgents only
	sm_respawn_wave_int_team_ins = CreateConVar("sm_respawn_wave_int_team_ins", "5", "Time in seconds bots will respawn in waves");
	...
	sm_vip_enabled = CreateConVar("sm_vip_enabled", "0", "Disable or Enable VIP features 0/1");
...
public Action:Timer_Enemies_Remaining(Handle:Timer)
{
	...
	if (validAntenna != -1 || g_jammerRequired == 0)
	{
		...
		Format(textToPrint, sizeof(textToPrint), "Total Enemies alive: %d", alive_insurgents + g_iRemaining_lives_team_ins);
		PrintHintTextToAll(textToPrint);
		//PrintToChatAll(textToPrintChat);
...
public Action:Timer_AmmoResupply(Handle:timer, any:data)
{
	...
	for (new client = 1; client <= MaxClients; client++)
	{
		...
		if (IsPlayerAlive(client) && team == TEAM_1_SEC)
		{
			...
			decl String:sWeapon[32];
			GetEdictClassname(ActiveWeapon, sWeapon, sizeof(sWeapon));
			if (GetClientButtons(client) && ((StrContains(sWeapon, "weapon_defib") > -1) || (StrContains(sWeapon, "weapon_knife") > -1) || (StrContains(sWeapon, "weapon_kabar") > -1)))
			{
