...
new iDefCredits = 12; //Default
new iDefCreditsConst = 12; //Constant that never changes, Used to scaled iDefCredits
...
public OnPluginStart()
{
	...
	hCvarYellChance = CreateConVar("fy_chance", "0.5", "Chance of Yelling [0-1]", FCVAR_NOTIFY | FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
public ScaleBuildPoints() {
	...
	//if (nAvailableSupplyPoint <= 12)
	//	nAvailableSupplyPoint = 12;
	//Credits 
	if (tEngCount > 0)
		//iDefCredits = ((nAvailableSupplyPoint * 2) / tEngCount);
		iDefCredits = ((iDefCreditsConst * 2) / tEngCount);
	else
		//iDefCredits = (nAvailableSupplyPoint * 2);
		iDefCredits = (iDefCreditsConst * 2);
}
...
public FindValidAntenna()
{
	...
	while ((prop = FindEntityByClassname(prop, "prop_dynamic_override")) != INVALID_ENT_REFERENCE)
	{
		...
		//if (StrEqual(propModelName, "models/sernix/ied_jammer/ied_jammer.mdl"))		//New
		if (StrEqual(propModelName, "models/static_fittings/antenna02b.mdl"))		//Old
		{
			return prop;
		}
...
public GetPropType(entity)
{
	...
	//else if (StrEqual(propModelName, "models/sernix/ied_jammer/ied_jammer.mdl"))		//new
	else if (StrEqual(propModelName, "models/static_fittings/antenna02b.mdl"))		//old
	{
		propType = 3;
	}
	//else if (StrEqual(propModelName, "models/sernix/ammo_cache/ammo_cache_small.mdl"))		//new
	else if (StrEqual(propModelName, "models/static_props/wcache_crate_01.mdl"))		//old
	{
		propType = 4;
	}
					...
					for(new i=0; i<=iPropNo[engineerCheck]; i++)
					{
						...
						{	
							...
							if (tPropTarget != -1)
							{
								...
								{
									if ((StrContains(sWeapon, "weapon_kabar") > -1) || (StrContains(sWeapon, "weapon_doi2ins_shovel") > -1))
									{
...
public Action:Timer_Construct(Handle timer, Handle pack)
{
	...
	if (client > 0 && vectDist < 0 || vectDist > 0 || !IsPlayerAlive(client) || IsClientTimingOut(client) || (!(StrContains(sWeapon, "weapon_kabar") > -1) && !(StrContains(sWeapon, "weapon_doi2ins_shovel") > -1)))
	{
	...
	if (client > 0 && g_ConstructRemainingTime[client] <= 0 && IsPlayerAlive(client) && !IsClientTimingOut(client) && (StrContains(g_client_last_classstring[client], "engineer") > -1) && ((StrContains(sWeapon, "weapon_kabar") > -1) || (StrContains(sWeapon, "weapon_doi2ins_shovel") > -1)))
	{
		g_ConstructRemainingTime[client] = g_ConstructDeployTime;
...
OnButtonPress(client, button, buttons)
{
   ...
   if(button == IN_SPRINT && buttons & IN_DUCK && buttons & IN_CANCEL && (StrContains(g_client_last_classstring[client], "engineer") > -1) && g_engInMenu[client] == false && ((StrContains(sWeapon, "weapon_kabar") > -1) || (StrContains(sWeapon, "weapon_doi2ins_shovel") > -1)))// && !(buttons & IN_FORWARD) && !(buttons & IN_ATTACK2) && !(buttons & IN_ATTACK))// & !IN_ATTACK2) 
   {
