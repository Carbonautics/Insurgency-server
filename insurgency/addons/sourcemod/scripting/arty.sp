...
public void OnPluginStart() {
	...
	//RegConsoleCmd("sm_fs", CmdCallFS, "Call fire support where you looking at.", 0);
	RegAdminCmd("sm_afs", CmdCallAFS, 0);										// HINT: test command

	HookEvent("weapon_fire", Event_WeaponFire, EventHookMode_Pre);
	HookEvent("round_start", Event_RoundStart);
	HookEvent("player_pick_squad", Event_PlayerPickSquad);
	AutoExecConfig(true, "arty");
	InitSupportCount();
}
...
public void OnMapStart() {
	...
	PrecacheSound("doi/player/artillery/requestartillery1.ogg");
	PrecacheSound("doi/player/artillery/requestartillery2.ogg");
	PrecacheSound("doi/player/artillery/outgoingartillery1.ogg");
	PrecacheSound("doi/player/artillery/outgoingartillery2.ogg");
	PrecacheSound("doi/player/artillery/outgoingartillery3.ogg");
	PrecacheSound("doi/player/artillery/outgoingartillery4.ogg");
	PrecacheSound("doi/player/artillery/outgoingartillery5.ogg");
	PrecacheSound("doi/player/artillery/outgoingartillery6.ogg");
	PrecacheSound("doi/player/artillery/outgoingartillery7.ogg");
	PrecacheSound("doi/player/artillery/outgoingartillery8.ogg");
...
public Action Event_WeaponFire(Event event, const char[] name, bool dontBroadcast) {
	...
	switch(GetRandomInt(1, 2))
		{
		case 1: EmitSoundToAll("doi/player/artillery/requestartillery1.ogg", client, SNDCHAN_VOICE, _, _, 1.0);
		case 2: EmitSoundToAll("doi/player/artillery/requrestartillery2.ogg", client, SNDCHAN_VOICE, _, _, 1.0);
		}
...
Action CmdCallAFS(int client, int args) {													// HINT: Fire to where client looking
	...
	float ground[3];
	if (GetAimGround(client, ground) && !ArtyActivated_n) {
		ground[2] += 20.0;
		if (CallFireSupport(client, ground)) {
			CountAvailableSupport[team]--;
			IsEnabledTeam[team] = false;
			CreateTimer(gCvarDelayNextSupport.FloatValue, Timer_EnableTeamSupport, team, TIMER_FLAG_NO_MAPCHANGE);
			ArtyActivated_n = true;
		}
	}
	else
	{
		PrintHintText(client, "The Arty is already called on a position. Wait until previous arty is finished.");
	}
	ArtyActivated_n = false;
	return Plugin_Handled;
}
...
public Action Timer_LaunchMissile(Handle timer, DataPack pack) {
	...
	if (ValidateClient(client)) {
		...
		if(IsPlayerAlive(client) && shells > 24){
			switch(GetRandomInt(1, 8))
			{
			case 1: EmitSoundToAll("doi/player/artillery/outgoingartillery1.ogg", client, SNDCHAN_VOICE, _, _, 1.0);
			case 2: EmitSoundToAll("doi/player/artillery/outgoingartillery2.ogg", client, SNDCHAN_VOICE, _, _, 1.0);
			case 3: EmitSoundToAll("doi/player/artillery/outgoingartillery3.ogg", client, SNDCHAN_VOICE, _, _, 1.0);
			case 4: EmitSoundToAll("doi/player/artillery/outgoingartillery4.ogg", client, SNDCHAN_VOICE, _, _, 1.0);
			case 5: EmitSoundToAll("doi/player/artillery/outgoingartillery5.ogg", client, SNDCHAN_VOICE, _, _, 1.0);
			case 6: EmitSoundToAll("doi/player/artillery/outgoingartillery6.ogg", client, SNDCHAN_VOICE, _, _, 1.0);
			case 7: EmitSoundToAll("doi/player/artillery/outgoingartillery7.ogg", client, SNDCHAN_VOICE, _, _, 1.0);
			case 8: EmitSoundToAll("doi/player/artillery/outgoingartillery8.ogg", client, SNDCHAN_VOICE, _, _, 1.0);
			}
		}
		SDKCall(fCreateRocket, client, "rocket_rpg7", pos, DOWN_VECTOR);
		if (shells > 1) {
			CreateTimer(0.75 + GetURandomFloat(), Timer_LaunchMissile, pack, TIMER_FLAG_NO_MAPCHANGE);
		}
	}
	ArtyActivated_n = false;
	return Plugin_Handled;
...
bool GetAimGround(int client, float vec[3]) {
	...
	Handle ray = TR_TraceRayFilterEx(pos, dir, MASK_SOLID, RayType_Infinite, TraceWorldOnly, client);

	if (TR_DidHit(ray)) {
		TR_GetEndPosition(pos, ray);
		CloseHandle(ray);

		ray = TR_TraceRayFilterEx(pos, DOWN_VECTOR, MASK_SOLID, RayType_Infinite, TraceWorldOnly, client);
		if (TR_DidHit(ray)) {
